...
1. **Accounts** — any new `/etc/passwd`/`/etc/group` entry since baseline; the new name is itself the finding, and for a user, its own culprit. Separately, `getent passwd` (which resolves through the *full* NSS chain — sssd/winbind against Active Directory, LDAP, whatever's configured) is diffed against the *raw* `/etc/passwd` file: any name `getent` reports that isn't in the raw file is, by construction, a domain-backed account, not a local one. Flagged with no culprit — see below for why.
2. **SUID/SGID binaries** — new setuid/setgid files in a bounded set of common bin directories (`/usr/{,local/}{bin,sbin}`, `/bin`, `/sbin`), not a full filesystem walk. Culprit is the file's current owner.
