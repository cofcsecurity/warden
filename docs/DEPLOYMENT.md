...
Use `warden verify-backups --record` to save a verification report for `status`. Use `--repair` to recover local objects and missing or corrupt archives from verified copies. The JSON report lists object copies and a `manifests` matrix by tier, generation, and source. Repair moves corrupt regular-file archives aside and reports their quarantine paths; it refuses valid conflicting generations and does not change the active baseline. If that baseline needs recovery, use `retrieve` after reviewing the available generation. The default command only reads and prints JSON; it does not save a report. A successful heartbeat is independent of backup health.
